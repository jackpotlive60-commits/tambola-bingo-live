...
function loadVoicePreset(gameId) {
  try {
    ...
    return VOICE_PRESETS.some((preset) => preset.id === saved) ? saved : "auto";
  } catch {
    return "auto";
  }
}
...
function chooseSpeechVoice(voices, preset) {
  ...
  const byIndianLanguage = voices.find((voice) =>
    /^(en-IN|hi-IN)/i.test(String(voice.lang || ""))
  );

  return byIndianLanguage || voices[0] || null;
}
                ...
                onClick={() => {
                  try {
                    ...
                    const preview = new SpeechSynthesisUtterance(
                      "Agla number hai 42! Dhyaan se dekhiye."
                    );
