   ...
   {r.map(x=>
    <div key={x.id} style={{
     ...
     {x.status==="pending"&&<>
      ...
      <button onClick={()=>update(x.id,"rejected")}>
       Reject
      </button>
     </>}
    </div>
...
function CreateGame({done}){
 const[name,setName]=useState("TambolaLive");
  ...
  try{
   ...
   const{
    ...
    .insert({
     ...
     // Required by your Supabase table.
   ...
   setErr(
    e.message||"Could not create game"
   );
  }finally{
   setBusy(false);
  }
