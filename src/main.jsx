...
function HostControlCentre({
  ...
  async function shareInvite() {
    try {
      ...
      ctx.fillText("TAMBOLA LIVE", 540, 130);
      ctx.font = "bold 54px Arial";
      ctx.fillText(game.game_name || "TambolaLive", 540, 220);

      ctx.textAlign = "left";
      ctx.font = "36px Arial";
      ctx.fillText(`Date: ${game.game_date || "-"}`, 100, 330);
      ctx.fillText(`Time: ${game.game_time || "-"}`, 100, 390);
      ctx.fillText(`Ticket Price: ₹${game.ticket_price || 0}`, 100, 450);
      ctx.fillText(`Available Tickets: ${game.ticket_limit || 0}`, 100, 510);

      ctx.font = "bold 42px Arial";
      ctx.fillText("PRIZES", 100, 610);
      ctx.font = "32px Arial";
      const prizes = (game.prizes || []).filter(p => p && p.name && p.amount !== "" && p.amount !== null && p.amount !== undefined);
      prizes.forEach((p, i) => ctx.fillText(`${i + 1}. ${p.name} - ₹${p.amount}`, 120, 680 + i * 55));

      ctx.textAlign = "center";
      ctx.font = "bold 38px Arial";
      ctx.fillText("JOIN THE GAME", 540, 1080);
      ctx.font = "26px Arial";
      ctx.fillText("Open the invitation link to join", 540, 1140);
      ctx.font = "20px Arial";
      ctx.fillText(inviteUrl, 540, 1230);

      const blob = await new Promise(resolve => canvas.toBlob(resolve, "image/png"));
      if (!blob) throw new Error("Poster could not be created");
      const posterFile = new File([blob], "tambola-game-poster.png", { type: "image/png" });

      if (navigator.share && navigator.canShare && navigator.canShare({ files: [posterFile] })) {
        await navigator.share({
          title: game.game_name || "TambolaLive",
          text: `Join my Tambola game: ${game.game_name || "TambolaLive"}\n\n${inviteUrl}`,
          files: [posterFile],
        });
        return;
      }
      ...
      console.error(error);
      alert("Could not share the poster. Please use Copy Link.");
    }
  }
